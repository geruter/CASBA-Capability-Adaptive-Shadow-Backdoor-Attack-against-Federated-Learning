...
class Helper:
    def __init__(self, current_time, params, name):
        # 初始化Helper类的实例，设置基本参数和文件夹路径，配置日志记录器。

        self.current_time = current_time  # 存储传入的当前时间，用于文件命名和日志记录
        self.target_model = None  # 目标模型，用于聚合更新
        self.local_model = None  # 本地模型，用于在客户端进行训练

        self.indices_per_participant = None
        self.gan_train_dataset = None
        self.gan_train_data = None
        self.indices_per_participant = None
        self.train_data = None  # 存储训练数据
        self.test_data = None  # 存储测试数据
        self.poisoned_data = None  # 存储可能被污染的数据
        self.test_data_poison = None  # 存储用于测试的被污染数据

        self.params = params  # 参数字典，包含各种配置选项
        self.name = name  # 实例名称，通常用于标识不同的客户端或实验设置
        self.best_loss = math.inf  # 初始化最优损失为无穷大，用于模型保存逻辑
        self.folder_path = f'saved_models/model_{self.name}_{current_time}'  # 设置模型保存的文件夹路径

        # 尝试创建文件夹，如果文件夹已存在，则记录一个信息日志
        # try:
        ...
        try:
            ...
            logger.error(f"Failed to create directory {self.folder_path}: {e}")

        # 配置日志记录器，包括日志文件和控制台输出
        logger.addHandler(logging.FileHandler(filename=f'{self.folder_path}/log.txt'))
        logger.addHandler(logging.StreamHandler())
        logger.setLevel(logging.DEBUG)  # 设置日志级别为DEBUG
        logger.info(f'current path: {self.folder_path}')  # 记录当前文件路径的日志

        # 如果参数字典中没有'environment_name'，则使用实例名称作为环境名称
    ...
    def foolsgold_update(self, target_model, updates):
        ...
        return names, wv, alpha
        
    def accumulate_weight_fool(self, weight_accumulator, epochs_submit_update_dict, state_keys, num_samples_dict,
                               weights):
        ...
        for i in range(0, len(state_keys)):
            ...
            for name, data in local_model_update_dict.items():
                ...
                weight_accumulator[name].add_(local_model_update_dict[name] * weights[i])
                update[name].add_(local_model_update_dict[name])
                detached_data = data.cpu().detach().numpy()
                # print(detached_data.shape)
                detached_data = detached_data.tolist()
